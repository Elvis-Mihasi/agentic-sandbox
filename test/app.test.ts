...
describe("runs api", () => {
  ...
  it("transitions a run from planned to running to done", async () => {
  ...
  it("returns 404 when transitioning an unknown run", async () => {
  ...
  it("rejects an invalid status value", async () => {
    const res = await request(app).patch("/api/runs/run-0001/status").send({ status: "paused" });
